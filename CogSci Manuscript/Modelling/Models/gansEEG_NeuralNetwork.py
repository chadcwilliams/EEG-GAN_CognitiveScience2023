...
augFilename = 'augmentedPredictions_NN_SynP050_Runs8000_Filtered_Features_TestClassification.csv'
empFilename = 'empiricalPredictions_NN_Runs8000_Features_TestClassification.csv'
####
...
for dataSampleSize in dataSampleSizes:
    for addSyntheticData in syntheticDataOptions:
        ...
        #Define search space            
            ...
            pScores.append(predictScore)
            #print('Prediction: ' + str(round(predictResults['macro avg']['f1-score']*100)) + '%')
            
            if addSyntheticData:
                toWrite = [str(dataSampleSize),str(run),str(dataEpochs),str(predictScore),str(time.time()-startTime),optimal_params.best_params_]
            else:
